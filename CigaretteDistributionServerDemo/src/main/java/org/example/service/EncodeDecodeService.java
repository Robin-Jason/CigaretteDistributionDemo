...
public class EncodeDecodeService {
    ...
    public List<String> encode(String cigCode, String cigName, String deliveryMethod, String deliveryEtype, 
                        List<DemoTestData> cigaretteRecords) {
    ...
    public String encodeForSpecificArea(String cigCode, String cigName, String deliveryMethod, String deliveryEtype, 
                                       String targetArea, List<DemoTestData> allCigaretteRecords) {
        ...
        Map<String, List<DemoTestData>> gradeGroups = groupRecordsByGradeSettings(allCigaretteRecords);
        
        for (Map.Entry<String, List<DemoTestData>> entry : gradeGroups.entrySet()) {
            List<DemoTestData> groupRecords = entry.getValue();
     ...
     * 解析后的编码表达式数据结构
     */
    public static class ParsedExpressionData {
    ...
    /**
     * 编码区域信息（已弃用，由encodeRegionsForGroup替代）
     */
    @Deprecated
    private String encodeRegions(String deliveryEtype, List<DemoTestData> records) {
    ...
    /**
     * 编码档位投放量信息（已弃用，由encodeGradeSequencesForGroup替代）
     */
    @Deprecated
    private String encodeGradeAllocations(List<DemoTestData> records) {
        if (records == null || records.isEmpty()) {
            return "";
        }

        // 按区域分组处理档位数据
        Map<String, List<String>> allocationsByRegion = new HashMap<>();
        
        for (DemoTestData record : records) {
            String region = record.getDeliveryArea();
            if (region == null) continue;

            // 提取档位数据
            BigDecimal[] grades = extractGrades(record);
            String allocationCode = encodeGradeSequences(grades);
            
            allocationsByRegion.computeIfAbsent(region, k -> new ArrayList<>()).add(allocationCode);
        }

        // 检查是否所有区域的档位设置都相同
        Set<String> uniqueAllocations = allocationsByRegion.values().stream()
                .flatMap(List::stream)
                .collect(Collectors.toSet());

        if (uniqueAllocations.size() == 1) {
            // 所有区域档位设置相同，返回单一编码
            return uniqueAllocations.iterator().next();
        } else {
            // 区域档位设置不同，返回最常见的编码模式
            return uniqueAllocations.stream().findFirst().orElse("");
        }
    }
     ...
     * 提取档位数据数组
     */
    private BigDecimal[] extractGrades(DemoTestData record) {
    ...
    /**
     * 从表达式中解码区域信息（旧方法，保留用于向后兼容）
     * @deprecated 使用 decodeRegionsFromExpressionWithEtype 替代
     */
    @Deprecated
    private String decodeRegionsFromExpression(String expression) {
        // 确定投放类型以选择正确的解码映射
        String deliveryEtype = determineDeliveryEtypeFromExpression(expression);
        return decodeRegionsFromExpressionWithEtype(expression, deliveryEtype);
    }
    ...
    private List<String> generateMultipleEncodedExpressions(String deliveryMethodCode, String etypeCode, 
                                                           String deliveryEtype, List<DemoTestData> records) {
        List<String> expressions = new ArrayList<>();
        
        // 按档位设置对区域进行分组
        Map<String, List<DemoTestData>> gradeGroups = groupRecordsByGradeSettings(records);
     ...
     * 档位设置完全相同的区域归为一组
     */
    private Map<String, List<DemoTestData>> groupRecordsByGradeSettings(List<DemoTestData> records) {
        Map<String, List<DemoTestData>> groups = new HashMap<>();
     ...
     * 生成区域的档位设置模式字符串，用于分组
     */
    private String generateGradePattern(DemoTestData record) {
        BigDecimal[] grades = extractGrades(record);
     ...
     * 为分组编码区域信息
     */
    private String encodeRegionsForGroup(String deliveryEtype, List<DemoTestData> groupRecords) {
     ...
     * 为分组编码档位投放量信息
     */
    private String encodeGradeSequencesForGroup(List<DemoTestData> groupRecords) {
        if (groupRecords == null || groupRecords.isEmpty()) {
            return "";
        }
        
        // 使用第一个记录的档位设置（因为同组内档位设置相同）
        DemoTestData representative = groupRecords.get(0);
