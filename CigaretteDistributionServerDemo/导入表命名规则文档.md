...
| **档位+城乡分类代码** | 按档位扩展投放 | 档位+城乡分类代码 | `3` | ❌ | `region_clientNum_3_1` | - |
| **档位+业态** | 按档位扩展投放 | 档位+业态 | `4` | ❌ | `region_clientNum_4_1` | - |
...
request.setIsBiWeeklyFloat(true); // 设置了双周上浮，但此类型不支持

// getSequenceNumber() 返回: 3
// getSubSequenceNumber() 返回: 1 (强制返回1，忽略双周上浮设置)
// getTableName() 返回: region_clientNum_3_1
```
...
**A**: 现在使用两位序号格式（主序号_子序号）。主序号对应投放类型，子序号区分双周上浮。只有按档位统一投放、档位+区县、档位+市场类型支持双周上浮区分。
...
### Q6: 双周上浮功能对哪些投放类型生效？
**A**: 只对按档位统一投放、档位+区县、档位+市场类型生效。档位+城乡分类代码和档位+业态不支持双周上浮，即使设置了该标识，子序号也会固定为1。
...
**文档版本**: v2.0  
**最后更新**: 2025年9月20日  
**更新内容**: 新增双周上浮区分功能，支持region_clientNum_{主序号}_{子序号}命名格式  
**审核状态**: ✅ 已审核通过
